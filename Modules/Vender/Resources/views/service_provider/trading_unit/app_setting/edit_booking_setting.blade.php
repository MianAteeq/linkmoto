    ...
    <div class="content-header bg-white">
        <div class="row" style="border-bottom: 3px solid #949494;">
            <div class="col-xl-12 col-12 bg-white headerbg" style="padding-left: 32px;padding-top: 13px;">
                <h3 class="h3">Edit Booking Information</h3>
                <div class="breadcrumb-wrapper col-12 p-0">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            <a style="color: black"
                                href="{{ url('vender/service/provider/trading/unit/app/setting/' . $trading_unit['id']) }}#collaptr_business_info">
                                Bookings
                            </a>
                        </li>
        ...
        <div class="col-md-9" id="contens"
            ...
            border-bottom-right-radius: 6px;"
                    ...
                    <div class="card-content">
                        <form action="{{ route('vender.service.provider.trading.unit.booking.setting.submit') }}"
                            ...
                            <div class="link-body" style="padding: 10px">
                                ...
                                <div class="form-group row">
                                    <label class="col-md-4 label-control" for="eventRegInput5">Booking start time *
                                        (?)</label>
                                    <div class="col-md-8 mx-auto">
                                        <select id="start_time" name="start_time" class="form-control"
                                            style="width: 30%;border-radius: 4px;">
                                            <option value="none" selected="" disabled="">Select Start Time
                                            </option>

                                            @foreach ($hours as $hour)
                                                <option value="{{ $hour }}"
                                                    @if ($trading_unit['app_setting'] != null) @if ($trading_unit['app_setting']['start_time'] == $hour)  selected @endif
                                                    @endif >{{ $hour }}</option>
                                            @endforeach
                                            {{--
                                ...
                                <div class="form-group row">
                                    <label class="col-md-4 label-control" for="eventRegInput5">Booking time intervals
                                        (minutes) * (?)</label>
                                    <div class="col-md-8 mx-auto">
                                        <div style="display: flex;">
                                            <i class="fa fa-minus" onclick="minusClick()"
                                                style=" margin-top: 2%; font-size: 17px;margin-left: 15px;"></i>
                                            <input type="text" id="interval" readonly="readonly"
                                                class="form-control"
                                                value="{{ $trading_unit['app_setting']['interval'] ?? 10 }}"
                                                onkeyup="lookup(this);" name="interval" placeholder="Interval"
                                                style="">
                                            <i class="fa fa-plus" onclick="plusClick()"
                                                style="margin-top: 2%; font-size: 17px;"></i>
                                        </div>
                                        ...
                                        {{-- </select> --}}
                                        <p class="text-danger interval"
                                            style="padding-left: 10px;width:100%;display: none;margin-bottom: -8px;">This
                                            Field is Required !</p>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="col-md-4 label-control" for="eventRegInput5">Booking End time *
                                        (?)</label>
                                    <div class="col-md-8 mx-auto">
                                        <select id="end_time" name="end_time" class="form-control"
                                            ...
                                            @foreach ($hour_end_time as $hour)
                                                <option value="{{ $hour }}"
                                                    @if ($trading_unit['app_setting'] != null) @if ($trading_unit['app_setting']['end_time'] == $hour)  selected @endif
                                                    @endif>{{ $hour }}</option>
                                            @endforeach
                                            {{--
        ...
        function renderItem(value) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    let html =
                        `<option value="none" selected="" id="end_time_append" disabled="">Select End Time</option>`;
