...
/* 1. Prevent JS from freezing the container height, ensuring the footer stays at the actual bottom */
#contens {
  height: auto !important;
  min-height: 400px;
  padding-bottom: 70px !important;
  /* Make room for absolute footer on desktop */
}
...
@media (max-width: 767px) {
  ...
  .col-md-9>.row:first-of-type h4 {
    margin-left: 0 !important;
    text-align: center;
  }

  /* Override absolute positioning for the footer on mobile so it flows naturally */
  .footers {
    position: relative;
    width: 100% !important;
    bottom: 0 !important;
  ...
  /* Remove padding needed for absolute footer on desktop */
  #contens {
    ...
    <div class="col-xl-12 col-12 bg-white headerbg" style="padding-left: 32px;padding-top: 13px;">
      ...
      <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"> Trade units
          </li>
...
<div class="row">
  <div class="col-md-3">
    ...
    <h4 class="h3" style="border-radius: 7px;border: 2px solid black;padding: 10px;font-weight: 600;
        font-size: 17px; "> <img src="/service_provider.png"
        style="width: 22px;margin-top: -5px;margin-right: 3px;">Service Provider</h2>
    ...
    <div id="contens" style="border: 2px solid black;border-radius: 6px;margin-bottom: 10px;padding-left: 0;
      ...
      <div class="row" style="margin-right: 0;margin-left: 0;">
        <div class="col-md-12" style="border-bottom: 2px solid black;">
          <h3 style="font-size: 20px; padding: 10px; margin-left: -11px; color: black;padding-bottom: 0px;">Trade units
          </h3>
        </div>
          ...
          <div class="table-responsive">
            <table class="table table-striped table-bordered zero-configuration">
              ...
              <tbody>
                ...
                @foreach ($trading_units as $unit)
                <tr>
                  <td>{{$loop->iteration}}</td>
                  <td>{{$unit['name']}}</td>
                  <td> @if($unit['trading_template']==1)
                    {{auth()->user()->profile->company_name}}

                    @endif
                    @if($unit['trading_template']==2)
                    {{auth()->user()->profile->company_name}} Trading as {{$unit['trading_name']['name']}}

                    @endif
                    @if($unit['trading_template']==3)
                    {{$unit['trading_name']['name']??''}}

                    @endif
                  </td>
                  <td>
                    @if($unit['operation_type'] === 'Both')
                    On-site<br>
                    Mobile
                    @else
                    {{ ucfirst($unit['operation_type']) }}
                    @endif
                  </td>
                  <td>{{$unit['email']}}</td>
                  <td>{{$unit['mobile']}}</td>
                  <td>{{$unit['status']}}</td>
                  <td><a href="{{route('vender.service.provider.trading.unit.edit',$unit['id'])}}"><i
                        class="ft-edit"></i></a> <a
                      href="{{route('vender.service.provider.trading.unit.view',$unit['id'])}}"><i
                        class="ft-eye"></i></a></td>
                </tr>
      ...
      <div class="footers">
        ...
        <a href="{{route('vender.service.provider.trading.unit.add')}}"> <button type="button"
