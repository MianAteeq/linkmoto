            ...
            <table id="users-list-datatable" class="table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td><span class="badge badge-primary" style="background-color: #f26622  !important">{{ $quotation['status']
                                            }}</span></td>
                  @else
                  <td><span class="badge badge-light">{{ $quotation['status'] }}</span></td>

                  @endif
                  <td><a href="{{route('vender.print.booking',$quotation['id'])}}"><i class="la la-eye"></i></a>
                  </td>
                </tr>
...
<style>
/* Structural alignment for 1024px and below */
@media (max-width: 1024px) {
  .users-list-wrapper .card-body .media {
    align-items: center;
  }
}

/* Structural alignment for 770px and below (Mobile/Tablet) */
@media (max-width: 770px) {

  /* Align filter form button properly when stacked */
  .users-list-filter form .align-items-center {
    margin-top: 1rem;
    justify-content: stretch;
  }

  /* Ensure table overflow is smooth and touch-friendly */
  .table-responsive {
    display: block;
    width: 100%;
    overflow-x: auto;
