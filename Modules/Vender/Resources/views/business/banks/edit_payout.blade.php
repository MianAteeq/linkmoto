...
@media (max-width: 770px) {
  ...
  /* Override inline styles that constrict mobile viewing */
  .col-md-9[style] {
      ...
      <div class="breadcrumb-wrapper col-12">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a style="color: black" href="{{ route('vender.bank') }}">Payout
              Account</a></li>
          <li class="breadcrumb-item">Edit Payout Account
          </li>
  ...
  <div class="col-md-3">
    <div style="border-radius: 7px;border: 2px solid black; ">
      <h4 class="h3" style="font-weight: 600; font-size: 17px;padding: 10px; ">
        <img src="/home.png" style="width: 22px;margin-top: -5px;"> Edit Payout Account
      </h4>
        ...
        <div id="accordionWrap1" role="tablist" aria-multiselectable="true">
          <div class="card accordion collapse-icon accordion-icon-rotate" style="box-shadow: none;">
            <a id="business_VAT" class="card-header info collapsed" data-toggle="collapse" href="#collapsebusiness_vat"
              ...
              <div class="card-title lead"> Bank account (?)
              </div>
            </a>
            <div id="collapsebusiness_vat" data-parent="#accordionWrap1" role="tabpanel" aria-labelledby="business_VAT"
              ...
              <div class="card-content">
                <div class="card-body" style="color:black">
                  Select a <strong>verified bank account</strong> to
                  receive payouts and refunds from
                  the platform. Only verified
                  accounts appear in this list. If you
                  don’t see your bank account,
                  please add and verify it first under
                  <strong> Business > Bank Accounts.</strong>
    ...
    <div class="row" style="margin-right: 0;margin-left: 0;">
      <div class="col-md-12" style="border-bottom: 2px solid black;">
        <h3 style="font-size: 20px; padding: 10px; margin-left: -11px; color: black;padding-bottom: 0px;">
          Payout Account Information </h3>

      </div>


    </div>
    <form action="{{ route('vender.payout.save') }}" id="contens" method="POST" enctype="multipart/form-data"
      id="contens"> @csrf
      <div class="link-body" style="padding: 10px">

        <div class="form-group row">
          <label class="col-md-4 label-control" for="eventRegInput5"> Bank account (?) *</label>
          <div class="col-md-8 mx-auto">
            <select name="bank_id" id="bank_id" class="form-control select2" style="max-width:60%!important">
              <option value="">Select Bank Account</option>
              @foreach ($banks as $bank)
              <option value="{{ $bank['id'] }}" @if ($bank['is_payout']==1) selected @endif>
                {{ $bank['bank_name'] }} -
                {{ $bank['account_name'] }}
              </option>
              @endforeach
...
$('#bank_id').on('change', function() {
  ...
  if (this.value) {
    ...
    let exist = banks.filter((item) => item.id == this.value);
    console.log(exist);
    if (exist.length > 0) {
      let html = `<span>${exist[0].bank_name}</span> <br>`;
      html += `<span>${exist[0].account_name}</span> <br>`;
      html += `<span>${exist[0].account_number}</span> <br>`;
      html += `<span>${exist[0].sort_code}</span> <br>`;

      $('.show_bank_info').html(html);
    }
  }
});
</script>
