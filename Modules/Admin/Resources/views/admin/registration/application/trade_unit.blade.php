        ...
        <div class="col-xl-12 col-12 bg-white headerbg" style="padding-left: 32px;padding-top: 13px;">
           ...
           <div class="breadcrumb-wrapper col-12">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">Trade Unit
                </li>
            </ol>
        </div>
        </div>
...
<div class="row">
    <div class="col-md-3">
        ...
        <p style="color: black;padding-left: 25px;">Status :  @if ($user['application_status']=="IN_REVIEW") Info in Review @else {{$user['application_status']}}  @endif</p>
          <p style="color: black;padding-left: 25px;">Verification status :  @if($user['profile']['trade_unit_status']==0)  Pending @elseif ($user['profile']['trade_unit_status']==1) Accepted  @else Decline @endif</p>

          @if($user['profile']['trade_unit_status']==0)
          <div class="footers" style="border-top: 2px solid black;padding: 10px ">
            <button class="btn btn-primary btn-block" onclick="window.location.href=`{{route('admin.trade.unit.verify',$user['id'])}}`">ACCEPT</button>
            <button class="btn btn-primary btn-block" onclick="window.location.href=`{{route('admin.trade.unit.decline',$user['id'])}}`">DECLINE</button>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-sm-7 text-secondary">
                                <span class="badge badge-success mt-1" style="padding: 0.5em 1.6em;background-color: #ff822f;">{{$user['profile']['operation_type']}}</span>
                            </div>
                        </div>
                        @if ($user['profile']['operation_type']!="Mobile")
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-7 text-secondary">
                                {{$user['site_address']['address_line_1']??'N/A'}} , {{$user['site_address']['address_line_2']??'N/A'}} , {{$user['site_address']['city']??'N/A'}}  , {{$user['site_address']['postcode']??'N/A'}}
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                            <div class="col-sm-5">
                                <h6 class="mb-0">Proof of site address</h6>
                            </div>
                            <div class="col-sm-7 text-secondary">
                                {{$user['site_address']['proof_name']??'N/A'}} <a class="btn btn-primary btn-primary_2" style="border-color: #ff6600 !important; background-color: #ff6600 !important;float: right;" target="_blank" href="{{URL::to($user['site_address']['proof']??'')}}">View</a>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-7 text-secondary">
                             @if($user['profile']['trade_unit_status']==0)  Pending @elseif ($user['profile']['trade_unit_status']==1) Accepted  @else Decline @endif
                            </div>
                        </div>
                        @if($user['profile']['trade_unit_status']==0)
                        <div class="ac-footer">
                            <a class="btn btn-primary btn-primary_2" style="float: right;"  href="{{route('admin.trade.unit.info.verify',$user['id'])}}">VERIFY</a>
